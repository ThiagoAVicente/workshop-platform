...
Creates an Amazon Aurora PostgreSQL Serverless v2 cluster with a writer instance, a hot standby reader, and an AWS Backup plan with configurable schedule.

## Architecture

- **Serverless v2**: Automatically scales compute capacity between configurable min/max ACUs
- **Writer instance**: Primary serverless instance handling all read/write traffic
- **Reader instance**: Hot standby serverless instance with `promotion_tier = 1` for automatic failover
...
| `aws_rds_cluster` | 1 | Aurora PostgreSQL Serverless v2 cluster |
| `aws_rds_cluster_instance` (writer) | 1 | Primary serverless instance |
| `aws_rds_cluster_instance` (reader) | 1 | Hot standby serverless instance |
...
| `min_capacity` | `number` | `0.5` | no | Minimum Serverless v2 capacity in ACUs |
| `max_capacity` | `number` | `4` | no | Maximum Serverless v2 capacity in ACUs |
